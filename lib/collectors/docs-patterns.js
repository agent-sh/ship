...
// Lazy-load the repo-intel artifact loader (still named `repo-map` upstream
// in agentsys/lib for historical reasons - it produces and consumes
// `repo-intel.json`). The resolver (lib/agentsys) finds the canonical
// agentsys install, so this plugin no longer ships its own copy.
let repoIntelModule = null;
let repoIntelLoadError = null;

/**
 * Get the repo-intel artifact loader module, loading it lazily via the
 * agentsys resolver. The returned API exposes `exists / load / init /
 * update / checkAstGrepInstalled` (the legacy `repo-map` shape - the
 * underlying file is `repo-intel.json`).
 *
